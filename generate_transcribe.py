...
def transcript_exists(directory, video_id, output_format):
    """Checks if a specific Whisper-generated transcript file already exists."""
    specific_transcript_filename = f"{video_id}.whisper.{AUTO_DETECT_SUFFIX}.{output_format}"
    specific_transcript_filepath = os.path.join(directory, specific_transcript_filename)

    if os.path.exists(specific_transcript_filepath):
        print(f"  Skipping: Specific transcript '{specific_transcript_filename}' already exists.")
        return True
    return False
...
def pipeline():
    ...
    for item_name in os.listdir(DATASET_DIR):
        ...
        if os.path.isdir(item_path):
            ...
            print(f"  Transcript will be generated for '{video_id}'. Initiating process...")
    ...
    print(f"Transcripts needed (or regenerated): {transcripts_needed}")
    print(f"Transcripts generated successfully: {transcribed_successfully}")
    print(f"Skipped (specific transcript already existed): {skipped_existing}")
